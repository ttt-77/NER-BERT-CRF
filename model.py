...
def log_sum_exp_batch(log_Tensor, axis=-1): # shape (batch_size,n,m)
    return torch.max(log_Tensor, axis)[0]+torch.log(torch.exp(log_Tensor-torch.max(log_Tensor, axis)[0].view(log_Tensor.shape[0],-1,1)).sum(axis)) #[batch_size, n]
...
class BERT_CRF_NER(nn.Module):
    ...
    def _forward_alg(self, feats):
        ...
        log_alpha = torch.Tensor(batch_size, 1, self.num_labels).fill_(-10000.).to(self.device) #[batch_size,1,num_labels]
    ...
    def _score_sentence(self, feats, label_ids):
        ...
        '''
        #label_id: [batch_size, num_labels]
        # T = self.max_seq_length
        T = feats.shape[1]
        batch_size = feats.shape[0]

        batch_transitions = self.transitions.expand(batch_size,self.num_labels,self.num_labels)
        batch_transitions = batch_transitions.flatten(1) #[batch_size, num_labels*num_labels]

        score = torch.zeros((feats.shape[0],1)).to(self.device) #[batch_size,1]
    ...
    def neg_log_likelihood(self, input_ids, segment_ids, input_mask, label_ids):
        ...
        #  - log[ p(Zt=tag1:t|X=w1:t) ] = - log[ p(X=w1:t,Zt=tag1:t)/p(X=w1:t) ] = log[p(X=w1:t)] - log[p(X=w1:t,Zt=tag1:t)]
